...
import AssistSpotlight from './assist-spotlight';
...
vi.mock('../leaderboard-card/leaderboard-card', () => ({
  default: ({ className, textColors, player }: any) => (
    <div data-testid="leaderboard-card" className={className}>
      ...
      <span data-testid="player-assists">{player.statToTrack}</span>
    </div>
  ),
}));
...
describe('GoalSpotlight', () => {
  ...
  it('displays top 5 goal scorers in descending order', () => {
    ...
    render(
      <TestWrapper mockData={mockData}>
        <AssistSpotlight selectedSeason={season} />
      </TestWrapper>
    );
    ...
    const assistElements = screen.getAllByTestId('player-assists');
    expect(assistElements[0]).toHaveTextContent('10'); 
    expect(assistElements[1]).toHaveTextContent('8'); 
    expect(assistElements[2]).toHaveTextContent('5'); 
    expect(assistElements[3]).toHaveTextContent('4');  
    expect(assistElements[4]).toHaveTextContent('3');  
  });
  ...
  it('truncates position to first character', () => {
    ...
    expect(positionElements[0]).toHaveTextContent('D'); 
    expect(positionElements[2]).toHaveTextContent('F'); 
  });
  ...
  it('converts player numbers to strings', () => {
    ...
    expect(numberElements[0]).toHaveTextContent('#7');
    expect(numberElements[1]).toHaveTextContent('#53');
  });
  ...
  it('uses first season stats when player has multiple seasons', () => {
    ...
    render(
      <TestWrapper mockData={mockData}>
        <AssistSpotlight selectedSeason='23/24' />
      </TestWrapper>
    );
