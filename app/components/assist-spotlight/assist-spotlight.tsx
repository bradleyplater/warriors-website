...
function getTop5Assists(players: Player[], selectedSeason: Season): IStatSpotlightData[] {
    ...
    return playerWithStatsInSeason
        .map(player => {
            ...
            return {
                ...
                statToTrack: selectedSeason === 'overall' ? statsToTrack.reduce((total, stat) => total + stat.assists, 0) : statsToTrack[0].assists
            };
        })
...
export default function AssistSpotlight({ selectedSeason }: IStatSpotlightProps) {
    const { data } = useData();
    
    const topAssists = getTop5Assists(data.players, selectedSeason);
