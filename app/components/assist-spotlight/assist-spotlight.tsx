...
function getTop5Assists(players: Player[]): IStatSpotlightData[] {
    return players.map(player => ({
        number: player.number.toString(),
        name: player.name,
        position: player.position.slice(0,1),
        statToTrack: player.stats[0].assists
    })).sort((a,b) => b.statToTrack - a.statToTrack).slice(0,5)
}

export default function AssistSpotlight() {
    const { data } = useData();
    
    const topScorers = getTop5Assists(data.players);
