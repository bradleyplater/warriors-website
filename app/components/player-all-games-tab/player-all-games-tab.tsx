...
const getAllGames = (playerId: string, results: Result[]) => {
  return results.filter((game) => game.roster.includes(playerId))
  .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
}

function getAllGoals(playerId: string, results: Result[]) {
...
function getAllAssists(playerId: string, results: Result[]) {
...
function getAllPoints(allGoals: number, allAssists: number) {
  return allGoals + allAssists;
}

function getAllPointPerGame(allPoints: number, allGames: Result[]) {
  return allGames.length > 0 ? allPoints / allGames.length : 0;
}

function getAllWins(allGames: Result[]) {
  return allGames.filter((game) => game.score.warriorsScore > game.score.opponentScore).length;
}

function getAllLosses(allGames: Result[]) {
  return allGames.filter((game) => game.score.warriorsScore < game.score.opponentScore).length;
}
...
export default function PlayerAllGamesTab({ playerId }: PlayerAllGamesTabProps) {
  const { data } = useData();

  const allGames = getAllGames(playerId, data.results);

  const allGoals = getAllGoals(playerId, allGames);
  const allAssists = getAllAssists(playerId, allGames);

  const allPoints = getAllPoints(allGoals, allAssists);
  const allPPG = getAllPointPerGame(allPoints, allGames);

  // Calculate all-time performance stats
  const allStats = {
    totalGoals: allGoals,
    totalAssists: allAssists,
    totalPoints: allPoints,
    totalPointsPerGame: allPPG,
    wins: getAllWins(allGames),
    losses: getAllLosses(allGames),
    totalGames: allGames.length
  };

  const mappedAllGames = mapAllGames(allGames, playerId);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
        <div className="bg-gradient-to-br from-slate-50 to-slate-100 p-4 rounded-lg text-center">
          <div className="text-2xl md:text-3xl font-bold text-slate-700">
            {allStats.totalGames}
          </div>
          <div className="text-sm text-slate-600 font-medium">Games</div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
          <h3 className="text-lg font-semibold text-gray-900">All Games ({allStats.totalGames} total)</h3>
        </div>
