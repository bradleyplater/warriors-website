...
function getTop5Points(players: Player[], selectedSeason: Season): IStatSpotlightData[] {
    ...
    return playerWithStatsInSeason
        .map(player => {
            ...
            return {
                ...
                statToTrack: selectedSeason === 'overall' ? statsToTrack.reduce((total, stat) => total + stat.points, 0) : statsToTrack[0].points
            };
        })
...
export default function PointsSpotlight({selectedSeason}: IStatSpotlightProps) {
    ...
    const topPoints = getTop5Points(data.players, selectedSeason);
